...
from models.database import Game, db # do pacote do flask, importe a classe Flask
...
def init_app(app):
    ...
    def estoque():
        if request.method == 'POST':
            ...
            new_game = Game(titulo=titulo, ano=ano, categoria=categoria, plataforma=plataforma, preco=preco, quantidade=quantidade)
    ...
    def editar(id):
        ...
        if request.method == 'POST':
            ...
            game.ano = int(request.form.get('ano'))
            game.categoria = request.form.get('categoria')
            game.plataforma = request.form.get('plataforma')
            game.preco = float(request.form.get('preco'))
            game.quantidade = int(request.form.get('quantidade'))
            db.session.commit()
            return redirect(url_for('estoque'))
